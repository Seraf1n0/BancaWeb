...
CREATE OR REPLACE FUNCTION sp_cards_create(
    ...
    p_cvv_encriptado VARCHAR,
    p_pin_encriptado VARCHAR,
    p_moneda int,
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM tipoTarjeta WHERE id = p_tipo) THEN
        RAISE EXCEPTION 'Tipo de tarjeta no válido';
    END IF;
...
BEGIN
    ...
    SELECT 
        ...
        t.limite_credito,
        t.saldo_actual,
        (t.limite_credito - t.saldo_actual) as disponible,
        t.fecha_creacion,
